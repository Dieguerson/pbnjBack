...
const yargedArgs = yargs(args).default({ PORT: 8080, MODE: 'fork' }).alias({ p: 'PORT', m: 'MODE'}).argv
const { PORT, MODE } = yargedArgs
...
const goServer = () => {
  ...
  app.get('/info', (req, res) => {
    ...
    const data = {
      ...
      path: process.cwd(),
      pid: process.pid,
      folder: '/' + rocess.cwd().split('\\')[process.cwd().split('\\').length - 1],
      cpus: cpus
    }
    setTimeout(() => {
      io.emit('info', data)
